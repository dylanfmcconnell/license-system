...
public class LicenseTypeRepository : IRepository<LicenseType, int>
{
    ...
    public async Task<LicenseType?> GetEntityById(int id)
    {
        ...
        licenseType.Category = await connection.QueryFirstOrDefaultAsync<LicenseCategory>(categorySql, new { licenseType.CategoryId });

        
        licenseType.LicenseClass = Type.GetType($"LicenseSystem.Models.{licenseType.LicenseClassName}") ?? typeof(License);
    ...
    public async Task<IEnumerable<LicenseType>> GetAllEntities()
    {
        ...
        var categories = (await connection.QueryAsync<LicenseCategory>(categoriesSql, new { Ids = categoryIds })).ToDictionary(c => c.Id);
