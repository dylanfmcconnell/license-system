...
public sealed class LicenseRepository : IRepository<License, string>
{
    ...
    public async Task<License?> GetEntityById(string id)
    {
        ...
        license.Applicant = await connection.QueryFirstOrDefaultAsync<Applicant>(
            applicantSql,
            new { license.ApplicantId }
        );

        var licenseTypeSql = "SELECT * FROM LicenseType WHERE Id = @LicenseTypeId";
        license.LicenseType = await connection.QueryFirstOrDefaultAsync<LicenseType>(
            licenseTypeSql,
            new { license.LicenseTypeId }
        );
