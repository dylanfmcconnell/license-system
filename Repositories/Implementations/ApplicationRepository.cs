...
public sealed class ApplicationRepository : IRepository<Application, int>
{
    ...
    public async Task<Application?> GetEntityById(int id)
    {
        ...
        application.Applicant = await connection.QueryFirstOrDefaultAsync<Applicant>(
            applicantSql,
            new { application.ApplicantId }
        );

        var licenseTypeSql = "SELECT * FROM LicenseType WHERE Id = @LicenseTypeId";
        application.LicenseType = await connection.QueryFirstOrDefaultAsync<LicenseType>(
            licenseTypeSql,
            new { application.LicenseTypeId }
        );

        if (application.LicenseId != null)
        {
            var licenseSql = "SELECT * FROM License WHERE Id = @LicenseId";
            application.License = await connection.QueryFirstOrDefaultAsync<License>(
                licenseSql,
                new { application.LicenseId }
            );
        }
    ...
    public async Task<IEnumerable<Application>> GetAllEntities()
    {
        ...
        var applicants = (
            await connection.QueryAsync<Applicant>(applicantsSql, new { Ids = applicantIds })
        ).ToDictionary(a => a.Id);
        ...
        var licenseTypes = (
            await connection.QueryAsync<LicenseType>(licenseTypesSql, new { Ids = licenseTypeIds })
        ).ToDictionary(lt => lt.Id);

        var licenseIds = applications
            .Where(a => a.LicenseId != null)
            .Select(a => a.LicenseId)
            .Distinct()
            .ToList();
        ...
        if (licenseIds.Any())
        {
            ...
            licenses = (
                await connection.QueryAsync<License>(licensesSql, new { Ids = licenseIds })
            ).ToDictionary(l => l.Id);
        }
        ...
        foreach (var application in applications)
        {
            ...
            if (
                ...
                && licenses.TryGetValue(application.LicenseId, out var license)
            )
